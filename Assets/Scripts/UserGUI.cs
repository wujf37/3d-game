...
public class UserGUI : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(Screen.width / 2 - 300, 40, 420, 50), "游戏目标：让所有牧师活下去！", fontStyle2);
        if(GUI.Button (new Rect (Screen.width-100, Screen.height-100, 65, 65), "Exit")){
            Application.Quit();
        }
        if(check == 1){    //success
            GUI.Label(new Rect(Screen.width / 2 - 50, 150, 60, 40), "你赢了!", fontStyle);
            ...
            GUI.Label(new Rect(Screen.width / 2 - 50, 150, 60, 40), "你输了!", fontStyle);
            if (GUI.Button(new Rect(Screen.width / 2 - 40, Screen.height-200, 70, 40), "Restart")){
                action.Restart();
                check = 0;
            }
